...
#pragma warning restore CA1031
            ...
            catch (HttpRequestException ex)
            {
                ...
                await this.AuditAsync(blobId, TimestampStatus.NetworkError, networkTimestampException.Message, ct);
            ...
            if (resp.Content.Headers.ContentType?.MediaType != "application/timestamp-reply")
            {
                ...
                await this.AuditAsync(blobId, TimestampStatus.Error, mediaTimestampException.Message, ct);
            ...
            {
                await this.AuditAsync(blobId, TimestampStatus.Error, validateTimestampException.Message, ct);

                throw validateTimestampException;
            }
            else
            {
                await this.AuditAsync(blobId, TimestampStatus.Success, null, ct);
            }
            ...
            CancellationToken ct)
        {
            ...
            cmd.CommandText =
                $@"INSERT INTO [dbo].[TimeStampRequestsAuditLog] (
                    ...
                    ,[Status]
                    ,[Description])
                VALUES (NULL, @blobId, GETDATE(), @status, @desc)";

...
/// <param name="IsEmailNotificationEnabled">Флаг за нотификация по имейл</param>
/// <param name="IsFullFeatured">Флаг за профил, използващ цялата функционалност на системата</param>
public record RegisterIndividualDO(
    string Identifier,
    string FirstName,
    string MiddleName,
    string LastName,
    string Email,
    string Phone,
    RegisterIndividualDOAddress Address,
...
public record RegisterIndividualDOAddress(
    string Residence,
    string? City,
    string? State,
    string? CountryIso);

public class RegisterIndividualDOValidator : AbstractValidator<RegisterIndividualDO>
{
    private readonly EsbClient esbClient;

    public RegisterIndividualDOValidator(EsbClient esbClient)
    {
        ...
        this.RuleFor(x => x.Address)
            .Cascade(CascadeMode.Stop)
            .NotNull()
            .SetValidator(new RegisterIndividualDOAddressValidator(this.esbClient));
    }
}

public class RegisterIndividualDOAddressValidator : AbstractValidator<RegisterIndividualDOAddress>
{
    private readonly EsbClient esbClient;

    public RegisterIndividualDOAddressValidator(EsbClient esbClient)
    {
        this.esbClient = esbClient;

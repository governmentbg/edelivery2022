...
namespace ED.Domain
{
    internal record EsbOpenMessageCommandHandler(
        ...
        : IRequestHandler<EsbOpenMessageCommand, EsbOpenMessageCommandResult?>
    {
        private const string NotificationEvent = "Esb:OnDeliver";

        public async Task<EsbOpenMessageCommandResult?> Handle(
            EsbOpenMessageCommand command,
            CancellationToken ct)
        {
            ...
            else if (message.MessageSummaryVersion == MessageSummaryVersion.V2)
            {
                ...
                {
                    string profileName =
                        await this.EsbMessageOpenQueryRepository.GetProfileNameAsync(
                            command.ProfileId,
                            ct);
            ...
            CancellationToken ct)
        {
            ...
            GetNotificationRecipientsOnDeliveryVO[] notificationRecipients =
                await this.EsbMessageOpenQueryRepository

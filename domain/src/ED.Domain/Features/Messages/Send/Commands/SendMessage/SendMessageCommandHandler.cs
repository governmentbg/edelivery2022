...
namespace ED.Domain
{
    internal record SendMessageCommandHandler(
        ...
        IMessageSendQueryRepository MessageSendQueryRepository,
        ...
        : IRequestHandler<SendMessageCommand, Unit>
    {
            ...
            CancellationToken ct)
        {
            ...
            string[] recipientGroupsNames =
                command.RecipientGroupIds.Any()
                ? await this.MessageSendQueryRepository.GetRecipientGroupsNamesAsync(
                    command.RecipientGroupIds,
                    ct)
                : Array.Empty<string>();

            int[] recipientGroupsProfileIds =
                command.RecipientGroupIds.Any()
                ? await this.MessageSendQueryRepository.GetRecipientGroupsProfileIdsAsync(
                    command.RecipientGroupIds,
                    ct)
                : Array.Empty<int>();

            int[] distinctRecipientProfileIds = recipientGroupsProfileIds
                .Concat(command.RecipientProfileIds)
                .Distinct()
                .ToArray();

            if (!distinctRecipientProfileIds.Any())
            {
                throw new Exception("Message should have at least one recipient");
            }
            ...
            GetProfileNamesVO[] profileNames =
                await this.MessageSendQueryRepository.GetProfileNamesAsync(
                    allProfileIds,
                    ct);
            ...
            string recipientsAsText = string.Join(
                ", ",
                recipientGroupsNames.Concat(recipientProfileNames));
            ...
            string senderIdentifier =
                await this.MessageSendQueryRepository.GetProfileIdentifierAsync(
                    command.SenderProfileId,
                    ct);
            ...
            CancellationToken ct)
        {
            GetTemporaryOrStorageBlobsVO[] temporaryBlobs =
                await this.MessageSendQueryRepository.GetTemporaryOrStorageBlobsAsync(
                    senderProfileId,
                    blobIds,
                    ct);
            ...
            CancellationToken ct)
        {
            GetMessageBlobsVO[] messageBlobs =
                await this.MessageSendQueryRepository.GetMessageBlobsAsync(
                    senderProfileId,
                    messageId,
                    blobIds,
                    ct);
            ...
            CancellationToken ct)
        {
            GetNotificationRecipientsVO[] notificationRecipients =
                await this.MessageSendQueryRepository

...
namespace ED.Domain
{
    partial class MessageOpenHORepository : IMessageOpenHORepository
    {
        ...
        {
            IMessageOpenQueryRepository.GetPdfAsRecipientVO pdfAsRecipient =
                await this.messageOpenQueryRepository.GetPdfAsRecipientAsync(
                    messageId,
                    profileId,
                    ct);
            ...
            string body)
        {
            Dictionary<Guid, (bool, string)> templateDictionary =
                this.ParseTemplateToDictionaryAsRecipient(templateContent);

...
{
    public class ListProfileMessageRequirement : IAuthorizationRequirement
    {
    }
    ...
    public class ListProfileMessageRequirementHandler
        : AuthorizationHandler<ListProfileMessageRequirement>
    {
        protected override Task HandleRequirementAsync(
            AuthorizationHandlerContext context,
            ListProfileMessageRequirement requirement)
        {
            ...
            IAuthorizationRequirement requirement)
        {
            ...
            if (loginId != null &&
                profileId != null &&
                this.HasListProfileMessageAccess(loginId.Value, profileId.Value))
            {
                return true;
            }
        ...
        private bool HasListProfileMessageAccess(int loginId, int profileId)
        {
            AuthorizationClient authorizationClient = GrpcClientFactory.CreateAuthorizationClient();

            var resp = authorizationClient.HasListProfileMessageAccess(
                new HasListProfileMessageAccessRequest
                {
                    LoginId = loginId,
                    ProfileId = profileId,
                });

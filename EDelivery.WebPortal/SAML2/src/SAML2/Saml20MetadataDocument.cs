...
namespace SAML2
{
    ...
    public class Saml20MetadataDocument
    {
        ...
        public Saml20MetadataDocument(XmlDocument entityDescriptor)
            : this()
        {
            string entityId = entityDescriptor.DocumentElement.Attributes["entityID"].Value;
            Saml2Config config = Saml2Section.GetConfig();
            IdentityProvider provider = config
                .IdentityProviders
                .FirstOrDefault(e => string.Equals(entityId, e.Id, StringComparison.InvariantCultureIgnoreCase));

            if (!provider.OmitMetadataSignatureCheck)
            {
